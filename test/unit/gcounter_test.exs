defmodule EdgeCrdtTest.Unit.GCounterTest do
  ...
  test "mutate/3 increments the local replica component and returns a minimal delta" do
    ...
    assert {:ok, {state1, delta}} = Crdt.mutate(GCounter, state, :inc, {"a", 1})
    assert 1 == Crdt.value(GCounter, state1)

    assert {:ok, {state2, delta2}} = Crdt.mutate(GCounter, state1, {:inc, 2}, {"a", 2})
  ...
  test "apply_delta/3 is monotonic per replica (max) and updates the total" do
    ...
    assert {:ok, {state, delta}} = Crdt.mutate(GCounter, state, {:inc, 5}, {"a", 1})
  ...
  test "join/2 takes per-replica max and is commutative" do
    ...
    assert {:ok, {left, _}} = Crdt.mutate(GCounter, left, {:inc, 2}, {"a", 1})
    assert {:ok, {right, _}} = Crdt.mutate(GCounter, right, {:inc, 5}, {"a", 1})
    assert {:ok, {right, _}} = Crdt.mutate(GCounter, right, {:inc, 1}, {"b", 1})
